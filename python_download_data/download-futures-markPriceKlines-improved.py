...
"""
Improved script to download markPriceKlines with enhanced date tracking and error handling.
Features:
...
class MarkPriceKlinesDownloadProgressTracker:
    """追蹤下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol, interval):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_{interval}_markPriceKlines_status.json")
    ...
    def get_dates_to_download(
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "markPriceKlines", symbol, interval
                )
            except Exception as e:
        ...
        print(f"   📊 {symbol} {interval} markPriceKlines 狀態:")
...
def download_daily_markPriceKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    # 初始化進度追蹤器
    progress_tracker = MarkPriceKlinesDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日 MarkPriceKlines 資料")
        ...
        for interval in intervals:
            ...
            if not dates_to_download:
                print(f"      ✅ {symbol} {interval} MarkPriceKlines 資料已是最新")
                continue
...
def download_monthly_markPriceKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月 MarkPriceKlines 資料")
...
def download_monthly_markPriceKlines(
    ...
    return download_monthly_markPriceKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
def download_daily_markPriceKlines(
    ...
    return download_daily_markPriceKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance MarkPriceKlines 資料下載器")
    print("=" * 50)
    ...
    try:
        ...
        if args.skip_monthly == 0:
            ...
            if need_monthly:
                print(f"\n🗓️ 開始下載月 MarkPriceKlines 資料...")
                download_monthly_markPriceKlines_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    args.intervals,
                ...
                print(f"✅ 月 MarkPriceKlines 資料下載完成")
            else:
                print("✅ 月資料已完整，跳過下載")
        ...
        if args.skip_daily == 0:
            ...
            if need_daily:
                print(f"\n📅 開始下載日 MarkPriceKlines 資料...")
                download_daily_markPriceKlines_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    args.intervals,
                ...
                print(f"✅ 日 MarkPriceKlines 資料下載完成")
            else:
                print("✅ 日資料已完整，跳過下載")
