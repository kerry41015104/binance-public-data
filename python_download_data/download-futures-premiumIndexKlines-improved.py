...
"""
Improved script to download premiumIndexKlines with enhanced date tracking and error handling.
Features:
...
class PremiumIndexKlinesDownloadProgressTracker:
    """追蹤下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol, interval):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_{interval}_premiumIndexKlines_status.json")
    ...
    def get_dates_to_download(
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "premiumIndexKlines", symbol, interval
                )
            except Exception as e:
        ...
        print(f"   📊 {symbol} {interval} premiumIndexKlines 狀態:")
...
def download_daily_premiumIndexKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    # 初始化進度追蹤器
    progress_tracker = PremiumIndexKlinesDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日 PremiumIndexKlines 資料")
        ...
        for interval in intervals:
            ...
            if not dates_to_download:
                print(f"      ✅ {symbol} {interval} PremiumIndexKlines 資料已是最新")
                continue
...
def download_monthly_premiumIndexKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月 PremiumIndexKlines 資料")
                ...
                for month in months:
                    try:
                        ...
                        if current_date >= start_date and current_date <= end_date:
                            path = get_path(
                                trading_type, "premiumIndexKlines", "monthly", symbol, interval
                            )
...
def download_monthly_premiumIndexKlines(
    ...
    return download_monthly_premiumIndexKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
def download_daily_premiumIndexKlines(
    ...
    return download_daily_premiumIndexKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance PremiumIndexKlines 資料下載器")
    print("=" * 50)
    ...
    try:
        ...
        print("\n=== 📊 PREMIUM INDEX KLINES 資料檢查 ===")
        
        if args.skip_monthly == 0:
            print("\n🗓️ 檢查月資料...")
            need_monthly = check_existing_files(
                args.type, "premiumIndexKlines", symbols, "monthly", 
                args.intervals, args.years, args.months, 
                None, args.startDate, args.endDate, args.folder
            )
            if need_monthly:
                print(f"\n🗓️ 開始下載月 PremiumIndexKlines 資料...")
                download_monthly_premiumIndexKlines_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    args.intervals,
                ...
                print(f"✅ 月 PremiumIndexKlines 資料下載完成")
            else:
                print("✅ 月資料已完整，跳過下載")
        ...
        if args.skip_daily == 0:
            ...
            need_daily = check_existing_files(
                args.type, "premiumIndexKlines", symbols, "daily", 
                args.intervals, None, None, 
                dates, args.startDate, args.endDate, args.folder
            )
            if need_daily:
                print(f"\n📅 開始下載日 PremiumIndexKlines 資料...")
                download_daily_premiumIndexKlines_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    args.intervals,
                ...
                print(f"✅ 日 PremiumIndexKlines 資料下載完成")
            else:
                print("✅ 日資料已完整，跳過下載")
