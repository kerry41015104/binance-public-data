...
"""
Improved script to download aggTrades with enhanced date tracking and error handling.
Features:
...
class AggTradeDownloadProgressTracker:
    """追蹤aggTrade下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_aggTrades_status.json")
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "aggTrades", symbol, None
                )
            except Exception as e:
...
def download_daily_aggTrades_improved(
    trading_type,
    symbols,
    num_symbols,
    dates,
    ...
    # 初始化進度追蹤器
    progress_tracker = AggTradeDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日aggTrades資料")
        ...
        if not dates_to_download:
            print(f"      ✅ {symbol} aggTrades資料已是最新")
            continue
        ...
        for batch_idx in range(total_batches):
            ...
            for date_str in batch_dates:
                try:
                    ...
                    file_name = f"{symbol.upper()}-aggTrades-{date_str}.zip"
...
def download_monthly_aggTrades_improved(
    trading_type,
    symbols,
    num_symbols,
    years,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月aggTrades資料")
            ...
            for month in months:
                try:
                    ...
                    if current_date >= start_date and current_date <= end_date:
                        ...
                        file_name = f"{symbol.upper()}-aggTrades-{year}-{month:02d}.zip"
...
def download_monthly_aggTrades(
    ...
    return download_monthly_aggTrades_improved(
        trading_type,
        symbols,
        num_symbols,
        years,
...
def download_daily_aggTrades(
    ...
    return download_daily_aggTrades_improved(
        trading_type,
        symbols,
        num_symbols,
        dates,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance aggTrades資料下載器")
    print("=" * 50)
    ...
    try:
        ...
        if args.skip_monthly == 0:
            print(f"\n🗓️ 開始下載月aggTrades資料...")
            download_monthly_aggTrades_improved(
                args.type,
                symbols,
                num_symbols,
                args.years,
            ...
            print(f"✅ 月aggTrades資料下載完成")

        # 日資料下載
        if args.skip_daily == 0:
            print(f"\n📅 開始下載日aggTrades資料...")
            download_daily_aggTrades_improved(
                args.type,
                symbols,
                num_symbols,
                dates,
