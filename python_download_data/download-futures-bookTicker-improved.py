...
"""
Improved script to download bookTicker with enhanced date tracking and error handling.
Features:
...
class BookTickerDownloadProgressTracker:
    """追蹤下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_bookTicker_status.json")
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "bookTicker", symbol, None
                )
            except Exception as e:
...
def download_daily_bookTicker_improved(
    trading_type,
    symbols,
    num_symbols,
    dates,
    ...
    # 初始化進度追蹤器
    progress_tracker = BookTickerDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日 BookTicker 資料")
        ...
        if not dates_to_download:
            print(f"      ✅ {symbol} BookTicker 資料已是最新")
            continue
        ...
        for batch_idx in range(total_batches):
            ...
            for date_str in batch_dates:
                try:
                    ...
                    file_name = f"{symbol.upper()}-bookTicker-{date_str}.zip"
...
def download_monthly_bookTicker_improved(
    trading_type,
    symbols,
    num_symbols,
    years,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月 BookTicker 資料")
            ...
            for month in months:
                try:
                    ...
                    if current_date >= start_date and current_date <= end_date:
                        ...
                        file_name = f"{symbol.upper()}-bookTicker-{year}-{month:02d}.zip"
...
# 原有函數保持向下相容
def download_monthly_bookTicker(
    trading_type,
    symbols,
    num_symbols,
    years,
    ...
    return download_monthly_bookTicker_improved(
        trading_type,
        symbols,
        num_symbols,
        years,
...
def download_daily_bookTicker(
    ...
    return download_daily_bookTicker_improved(
        trading_type,
        symbols,
        num_symbols,
        dates,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance BookTicker 資料下載器")
    print("=" * 50)
    ...
    try:
        ...
        if args.skip_monthly == 0:
            ...
            if need_monthly:
                print(f"\n🗓️ 開始下載月 BookTicker 資料...")
                download_monthly_bookTicker_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    args.years,
        ...
        if args.skip_daily == 0:
            ...
            if need_daily:
                print(f"\n📅 開始下載日 BookTicker 資料...")
                download_daily_bookTicker_improved(
                    args.type,
                    symbols,
                    num_symbols,
                    dates,
