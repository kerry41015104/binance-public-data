...
"""
Improved script to download indexPriceKlines with enhanced date tracking and error handling.
Features:
- Enhanced date tracking for each symbol and interval
...
class IndexPriceKlinesDownloadProgressTracker:
    """追蹤indexPriceKlines下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol, interval):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_{interval}_indexPriceKlines_status.json")
    ...
    def get_dates_to_download(
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "indexPriceKlines", symbol, interval
                )
            except Exception as e:
                print(f"   ⚠️ 獲取 {symbol} {interval} 網頁最早日期失敗: {e}")
            ...
            try:
                ...
                print(f"   ⚠️ 解析 {symbol} {interval} 網頁最早日期失敗: {e}")
                start_date_from_web = start_date
        ...
        print(f"   📊 {symbol} {interval} indexPriceKlines 狀態:")
...
def download_daily_indexPriceKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    """改進的日indexPriceKlines資料下載函數"""
    current = 0

    # 確保 folder 路徑正確
    folder = ensure_folder_path(folder)
    # 初始化進度追蹤器
    progress_tracker = IndexPriceKlinesDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日indexPriceKlines資料")
        ...
        for interval in intervals:
            ...
            if not dates_to_download:
                print(f"      ✅ {symbol} {interval} indexPriceKlines資料已是最新")
                continue
...
def download_monthly_indexPriceKlines_improved(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月indexPriceKlines資料")
                ...
                for month in months:
                    try:
                        ...
                        if current_date >= start_date and current_date <= end_date:
                            ...
                            print(f"    📅 {year}-{month:02d} {interval}", end="")
                            success = download_file(
                        ...
                        print(f"    📅 {year}-{month:02d} {interval} ❌ 錯誤: {str(e)}")
                        continue
...
def download_monthly_indexPriceKlines(
    ...
    return download_monthly_indexPriceKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
def download_daily_indexPriceKlines(
    ...
    return download_daily_indexPriceKlines_improved(
        trading_type,
        symbols,
        num_symbols,
        intervals,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance indexPriceKlines資料下載器")
    print("=" * 50)
    ...
    try:
        ...
        if args.skip_monthly == 0:
            print(f"\n🗓️ 開始下載月indexPriceKlines資料...")
            download_monthly_indexPriceKlines_improved(
                args.type,
                symbols,
                num_symbols,
                args.intervals,
            ...
            print(f"✅ 月indexPriceKlines資料下載完成")

        # 日資料下載
        if args.skip_daily == 0:
            print(f"\n📅 開始下載日indexPriceKlines資料...")
            download_daily_indexPriceKlines_improved(
                args.type,
                symbols,
                num_symbols,
                args.intervals,
            ...
            print(f"✅ 日indexPriceKlines資料下載完成")
