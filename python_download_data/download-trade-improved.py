...
"""
Improved script to download trades with enhanced date tracking and error handling.
Features:
...
class TradeDownloadProgressTracker:
    """追蹤下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_trades_status.json")
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "trades", symbol, None
                )
            except Exception as e:
...
def download_daily_trades_improved(
    trading_type,
    symbols,
    num_symbols,
    dates,
    ...
    # 初始化進度追蹤器
    progress_tracker = TradeDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日交易資料")
        ...
        for batch_idx in range(total_batches):
            ...
            for date_str in batch_dates:
                try:
                    ...
                    file_name = f"{symbol.upper()}-trades-{date_str}.zip"
...
def download_monthly_trades_improved(
    trading_type,
    symbols,
    num_symbols,
    years,
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的月交易資料")
...
def download_monthly_trades(
    ...
    return download_monthly_trades_improved(
        trading_type,
        symbols,
        num_symbols,
        years,
...
def download_daily_trades(
    ...
    return download_daily_trades_improved(
        trading_type,
        symbols,
        num_symbols,
        dates,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.skip_monthly == 0:
            print(f"\n🗓️ 開始下載月交易資料...")
            download_monthly_trades_improved(
                args.type,
                symbols,
                num_symbols,
                args.years,
        ...
        if args.skip_daily == 0:
            print(f"\n📅 開始下載日交易資料...")
            download_daily_trades_improved(
                args.type,
                symbols,
                num_symbols,
                dates,
