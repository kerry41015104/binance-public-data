...
"""
Improved script to download bookDepth with enhanced date tracking and error handling.
Features:
...
class BookDepthDownloadProgressTracker:
    """追蹤下載進度和日期記錄"""
    ...
    def get_symbol_status_file(self, trading_type, symbol):
        ...
        return os.path.join(symbol_dir, f"{symbol.upper()}_bookDepth_status.json")
        ...
        if status["latest_date"]:
            ...
            try:
                start_date_from_web = self.scraper.get_earliest_date_for_symbol(
                    trading_type, "bookDepth", symbol, None
                )
            except Exception as e:
...
def download_daily_bookDepth_improved(
    trading_type,
    symbols,
    num_symbols,
    dates,
    ...
    # 初始化進度追蹤器
    progress_tracker = BookDepthDownloadProgressTracker(folder)
    ...
    for symbol in symbols:
        current += 1
        print(f"\n[{current}/{num_symbols}] - 開始處理 {symbol} 的日 BookDepth 資料")
        ...
        if not dates_to_download:
            print(f"      ✅ {symbol} BookDepth 資料已是最新")
            continue
        ...
        for batch_idx in range(total_batches):
            ...
            for date_str in batch_dates:
                try:
                    ...
                    file_name = f"{symbol.upper()}-bookDepth-{date_str}.zip"
...
def download_daily_bookDepth(
    ...
    return download_daily_bookDepth_improved(
        trading_type,
        symbols,
        num_symbols,
        dates,
...
if __name__ == "__main__":
    print("🚀 啟動改進版 Binance BookDepth 資料下載器")
    print("=" * 50)
    ...
    if args.type not in ["um", "cm"]:
        raise_arg_error("BookDepth is only for the um or cm trading type")
    ...
    try:
        ...
        if need_daily:
            print(f"\n📅 開始下載日 BookDepth 資料...")
            download_daily_bookDepth_improved(
                args.type,
                symbols,
                num_symbols,
                dates,
